...
class InkYamlRepresenter : Representer() {
  ...
  protected fun getComments(property: Property?): List<CommentLine> {
    ...
    val comments: bot.inker.api.config.Comments? = property.getAnnotation(bot.inker.api.config.Comments::class.java)
    if (comments == null) {
      val comment: bot.inker.api.config.Comment? = property.getAnnotation(bot.inker.api.config.Comment::class.java)
      if (comment != null) {
    ...
    val kotlinComment: bot.inker.api.config.KotlinComment? =
      property.getAnnotation(bot.inker.api.config.KotlinComment::class.java)
